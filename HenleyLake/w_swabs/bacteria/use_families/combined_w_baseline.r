...
# Call this function for ribs, scapula, and combined ribs/scapulae.  Find the
# axes limits for the line plots to identify consistent axes which accommodate
# the lowest/highest values across all 3 models.
...
lineXrng <- c(0, max(ribSummTopT$degdays,
                  scapSummTopT$degdays,
                  bothSummTopT$degdays) )
lineYrng <- c(0, max(ribSummTopT$meanPercByDay,
...
riblinePanel <- ggplot(ribSummTopT, aes(x=degdays, y=meanPercByDay, group=taxon)) +
  ...
  scale_x_continuous(limits=lineXrng, expand=c(0,0)) +
  scale_y_continuous(limits=lineYrng, expand=c(0,0)) +
  theme_minimal() +
...
ggsave(file="hl_combined_family_w_baseline_6panels.pdf", height=7.5,
  width=7.5, units="in")
# ########################
# ##################################################
...
bothpredvactT <- read_csv("both_ribs_scapulae/w_baseline/predicted_actual_w_type.csv")
...
ggsave(file="hl_combined_family_w_baseline_predicted_vs_actual_ADD.pdf",
