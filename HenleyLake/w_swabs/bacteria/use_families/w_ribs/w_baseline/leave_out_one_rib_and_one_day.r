...
wideT$ribnum <- substring(wideT$sampleName, first=2, last=2)
...
# each split is about 13 (but 12 and 14 are so close that it's debatable which
# of these is best) for the response variable in the original units.
numVarSplit <- 13
## ##################################################
...
for (i in 1:numCVs){
  lvOut <- (wideT$ribnum==pull(excludeT[i,"ribnum"])) |
