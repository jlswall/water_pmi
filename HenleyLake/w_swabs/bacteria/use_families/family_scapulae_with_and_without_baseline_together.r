...
wBaselineT <- allT %>% filter(type=="Scapula")
...
# abundance graphs).
plotrow2 <- plot_grid(noBaselinebarPanel, noBaselineLinePanel,
...
ggsave(file="hl_scapula_with_and_without_baseline_4panels.pdf", height=5,
  width=7.5, units="in")
# ########################
# ##################################################
...
# Make a tibble of actual and predicted values for each observation.
predvactT <- as_tibble(data.frame(predicted=noBaselineRF$predicted,
  actual=noBaselineRF$y))
predvactT$resids <- with(predvactT, actual - predicted)
# Force Rsq to be printed to 2 decmial places.
Rsq <- with(predvactT, format(round(cor(actual, predicted)^2, 2), nsmall=2))
# RMSE around 1:1 line, not regression line.
...
ggsave(file="hl_scapula_family_with_and_without_baseline_predicted_vs_actual_ADD.pdf",
