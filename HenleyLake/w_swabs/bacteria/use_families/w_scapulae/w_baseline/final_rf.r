...
# Move back to wide format.  There are 28 samples (including 2 at ADD 0) and 37
# taxa.
wideT <- taxaT %>%
  filter(taxon!="Rare") %>%
...
set.seed(542060)
fullResultsL <- mclapply(repDataL, mc.cores=8, fitFullDataF, mtry=numVarSplit,
  ntree=numBtSamps)
...
# RMSE: 589.4529  18.3318
c(mean(fullRsq), 1.96*sd(fullRsq))
# Rsq: 0.81243622 0.01166826
...
# From sampleName variable, extract the scapula number.  Include the scapula
