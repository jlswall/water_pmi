...
wideT$scapnum <- substring(wideT$sampleName, first=2, last=2)
...
# We exclude in turn each combination of individual scapula and degree day.
...
for (i in 1:numCVs){
  lvOut <- (wideT$scapnum==pull(excludeT[i,"scapnum"])) |
    (wideT$degdays==pull(excludeT[i,"degdays"]))
...
origUnitsF <- function(x, mtry, ntree){
  rf <- randomForest(degdays ~ ., data=x$trainT, mtry=mtry, ntree=ntree,
    importance=T)
  return(predict(rf, newdata=x$validT))
}

# Set random seed for reproducibility.
set.seed(4086165)   # For 1 run per combo
...
# Write this info out.
write.csv(residDF, file="resids_leave_out_one_scapula_and_one_day.csv",
  row.names=FALSE)
# #########################################
...
# This is about 970-973, whether I use 1, 10, or 100 runs per
# combo.
# #########################################
...
ggsave(filename="leave_out_one_scapula_and_one_day_residuals.pdf", height=3.5,
  width=3.5, units="in")
