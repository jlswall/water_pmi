...
## Move back to wide format.  There are 87 samples and 34 taxa.
wideT <- taxaT %>%
  filter(taxon!="Rare") %>%
...
## RMSE: 331.900252   4.474637
c(mean(fullRsq), 1.96*sd(fullRsq))
## Rsq: 0.952118105 0.001290759
...
ggsave(filename="families_scapula_no_baseline_PercIncMSE_barchart.pdf", height=4.5, width=6, units="in")
## ##################################################
...
ggsave("infl_scapula_family_no_baseline_scatter.pdf", width=8, height=4, units="in")
## ##################################################
