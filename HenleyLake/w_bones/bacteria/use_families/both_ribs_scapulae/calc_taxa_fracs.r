...
# ##################################################
# We want only rib and scapulae data, so we exclude the samples from the water.
allT <- allT %>% filter(type %in% c("Rib", "Scapula"))
# ##################################################
...
# There are 23 such taxa (not including the "Rare" group).
...
# type, but not both.  Such taxa are not present in large quantities (less than
# 10% on any one collection).
...
write.csv(allT, file="bones_combine_rib_scapula_massaged.csv", row.names=FALSE)
# ##################################################
