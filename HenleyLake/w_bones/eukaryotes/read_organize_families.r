...
fileNm <- "orig_data_files/Henley_Lake_18S_Taxonomy_2019-12-30.xlsx"

## Rows 193-197 contain summary data ("Eukaryota", "Total
## Unclassified", etc.), so we exclude this info.  Rows 191-192 are
## empty, and row 1 is the header.  We should have 189 rows of data.
## Note that row 2 contains counts of all eukaryotes.
rawAllT <- read_excel(path=fileNm, sheet="Tax Level 5-Family", n_max=189)
rm(fileNm)
...
## Note that for sampleName HLS1C17CS the ADD wasn't entered properly
## in the sample information file.  We use the same ADD that was used
## for the other samples taken during collection 17.
samplingT$degdays[samplingT$sampleName=="HLS1C17CS"] <- 4527
...
## dashes with underscores.  This seems to only be a problem for taxon
## "MAST-2_fa".
