...
plotrow1 <- annotate_figure(plotrow1,
  left=text_grob("Using bones, with baseline obs", face="bold", rot=90, size=10))

# Second row shows info about taxa in second analysis (bar chart, relative
# abundance graphs).
plotrow2 <- plot_grid(wSwabsbarPanel, wSwabsLinePanel, rel_widths=c(1, 2), nrow=1)
plotrow2 <- annotate_figure(plotrow2,
  left=text_grob("Using swabs, with baseline obs", face="bold", rot=90, size=10))
