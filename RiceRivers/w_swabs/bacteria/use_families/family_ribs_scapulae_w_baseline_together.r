...
# Retain the baseline observations (ADD 0).
...
# which are less influential, and don not appear in the top "m" taxa for either
...
# Leave day 0 in when using baseline obs in analyses.
summTopT <- chooseT %>%
  # filter(degdays > 0) %>%  # These analyses used baseline obs
...
ggsave(file="rr_swabs_rib_scapula_family_w_baseline_4panels.pdf", height=5,
  width=7.5, units="in")
# ########################
# ##################################################
...
ggsave(file="rr_swabs_rib_scapula_family_w_baseline_predicted_vs_actual_ADD.pdf",
