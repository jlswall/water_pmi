...
## Filter the data to just that type, and remove baseline observations
## (degdays=0) which happen before samples placed in water.
...
## distinguish data belonging to individual scapulae.
wideT <- taxaT %>%
  filter(taxon!="Rare") %>%
...
wideT$scapnum <- substring(wideT$sampleName, first=3, last=4)
...
## to consider at each split is about 20. (With numBtSamps==3000, this
## has the smallest avgcvMSE and 3rd lowest avgcvErrFrac.)  However,
## using values of 16-21 for numVarSplit yielded very similar results.
numVarSplit <- 20
## ##################################################
...
## We exclude in turn each combination of individual scapula and degree
## day. If there were no missing observations, this would be 24 degree
## days x 5 individual scapulae = 120 combos.  However, we have no
## observations on 10 different combos of scapulae and degdays.
excludeCombos <- wideT %>% select(degdays, scapnum) %>% distinct()
...
rm(excludeCombos)
## Ensure ordering is by degdays and then scapnum.
excludeT <- excludeT %>% arrange(degdays, scapnum)
...
for (i in 1:numCVs){
  lvOut <- (wideT$scapnum==pull(excludeT[i,"scapnum"])) | (wideT$degdays==pull(excludeT[i,"degdays"]))
  trainT <- wideT[!lvOut,] %>% select(-scapnum, -sampleName)
  validT <- wideT[lvOut,]
...
for (i in 1:numCVs){
  ...
  iresidDF <- data.frame(dayOmit=pull(excludeT[i,"degdays"]),
                         scapnumOmit=pull(excludeT[i,"scapnum"]),
                         scapnumactual=validT$scapnum,
...
## Write this info out.
write.csv(residDF, file="resids_leave_out_one_scap_and_one_day.csv", row.names=FALSE)
## #########################################
...
## the model makes for a scapula and time slot that we're never
...
## 1 run per combo: 638.3317
## 10 runs per combo: 637.1648
## 100 runs per combo: 637.1495
## This is about 637-639, whether I use 1, 10, or 100 runs per
## combo.
## #########################################
...
ggplot(residDF %>%
       ...
       filter(scapnumOmit==scapnumactual),
       aes(x=yactual, y=resid)) +
  geom_point() +
  ## geom_point(aes(col=scapnumOmit)) +
  geom_hline(yintercept=0) +
  labs(x="Actual degree days", y="Error (actual - estimated)")
ggsave(filename="leave_out_one_scap_and_one_day_residuals.pdf", height=3.5, width=3.5, units="in")

ggplot(residDF, aes(x=yactual, y=resid)) +
  facet_wrap(~scapnumOmit) +
  geom_point(aes(col=scapnumOmit)) +
