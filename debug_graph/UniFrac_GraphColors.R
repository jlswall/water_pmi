...
## JLS added: Only the first 6 columns have data; the last 9 columns
## contain only NAs.  We remove these columns.
meta <- meta[,1:6]
...
## JLS: The column ADDCollectionDays does not seem to be include in
## the file "HenleyLake_Scapula_Season_R_microbiota.txt".  I can see
## the first 2 elements are the ADD and collection number, so I will
## re-create that portion based on a combination of the
## Actual.Collected.ADD and Collection columns.
meta$ADDCollectionDays <- paste(meta$Actual.Collected.ADD, meta$Collection, sep="/")
