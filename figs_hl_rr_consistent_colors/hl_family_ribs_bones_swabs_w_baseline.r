...
load(paste0("../", location, "/w_bones/bacteria/use_families/w_ribs/w_baseline/families_ribs_rfmodel.RData"))
# The object was named "rf", but we don't want to mix up the 2 models being
# compared.  So we re-name it.
wBonesRF <- rf
rm(rf)

load(paste0("../", location, "/w_swabs/bacteria/use_families/w_ribs/w_baseline/families_ribs_rfmodel.RData"))
...
# For the title.
topTitle <- ggdraw() + draw_label("Ribs", fontface="bold")
...
ggsave(file="hl_indic_taxa_rib_bones_vs_swabs_with_baseline.pdf", height=5,
       width=7.5, units="in")
# ########################
# ##################################################
