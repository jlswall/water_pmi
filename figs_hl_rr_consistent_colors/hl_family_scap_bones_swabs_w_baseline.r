...
# Filter data to just scapulae.
bonesT <- allT %>% filter(type=="Scapula")
...
# Filter data to just scapulae.
swabsT <- allT %>% filter(type=="Scapula")
...
load(paste0("../", location, "/w_bones/bacteria/use_families/w_scapulae/w_baseline/families_scapulae_rfmodel.RData"))
# The object was named "rf", but we don't want to mix up the 2 models being
# compared.  So we re-name it.
wBonesRF <- rf
rm(rf)

load(paste0("../", location, "/w_swabs/bacteria/use_families/w_scapulae/w_baseline/families_scapulae_rfmodel.RData"))
...
# For the title.
topTitle <- ggdraw() + draw_label("Scapulae", fontface="bold")
...
ggsave(file="hl_indic_taxa_scapulae_bones_vs_swabs_with_baseline.pdf", height=5,
       width=7.5, units="in")
# ########################
# ##################################################
